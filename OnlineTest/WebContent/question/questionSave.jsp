...
<form action="question!saveQuestion" method="post"
	onsubmit="return checkForm()">
	<table width="90%" align="center">
		<tr>
			<td><label>考试题目：</label></td>
			<td><input type="text" id="subject" name="question.subject"
			...
			<td><select id="paperName" name="question.paper.id">
					...
					<c:forEach var="paper" items="${paperList }">
						<option value="${paper.id }"
							${paper.id==question.paper.id?'selected':'' }>${paper.paperName }</option>
					</c:forEach>
			</select></td>
		</tr>
		<tr>
			<td><label>题目难度：</label></td>
			<td><select id="level" name="question.level">
					<option value="">请选择题目难度...</option>
					<option value="难" ${question.level=='难'?'selected':'' }>难</option>
					<option value="中" ${question.level=='中'?'selected':'' }>中</option>
					<option value="易" ${question.level=='易'?'selected':'' }>易</option>
			</select></td>
		</tr>
		<tr>
			<td><input type="hidden" id="joinTime" name="question.joinTime"
		...
		<tr>
			<td><input type="hidden" id="questionId" name="questionId"
